...
public final class Image implements ActionListener {
    ...
    /**
     * Creates a new image from an array of colors
     *
     * @param colorArray an array of colors
     * @param width      the width of colorArray
     * @return image consisting of pixels that match the colorArray
     */
    public static Image makeImageFromColorArray(Color[][] colorArray, int width) {
        Image newImage = new Image(width, colorArray.length);

        for (int i = 0; i < colorArray.length; i++) {
            for (int j = 0; j < width; j++) {
                newImage.set(j, i, colorArray[i][j]);
            }
        }
    ...
    /**
     * Creates an image that is all one color
     *
     * @param color  the color that the image will be
     *               is not null
     * @param width  the width of the image
     *               is greater than 0
     * @param height the height of the image
     *               is greater than 0
     * @return an image of width and height, that consists
     * entirely of pixels of color
     */
    public static Image makeSolidColorImage(Color color, int width, int height) {

        Color[][] colorMatrix = new Color[height][width];

        for (int i = 0; i < width; i++) {
            for (int j = 0; j < height; j++) {
                colorMatrix[j][i] = color;
            }
        }

        Image solidImage = makeImageFromColorArray(colorMatrix, width);

        return solidImage;
    }

    /**
     * Changes all of the pixels specified pixels within an image to a certain color
     * modifies this
     *
     * @param pixels      a set of pixels to be changed represented by Points
     * @param changeColor the color that the pixels will be changed too
     */
    public void changeColourOfPixelSet(HashSet<Point> pixels, Color changeColor) {
        for (Point p : pixels) {
            this.set(p.x, p.y, changeColor);
        }
    }
